...
/**
 * La colección sigue llamándose `cost-types` en Firestore: renombrarla
 * obligaría a migrar los documentos y nadie la ve desde la app.
 */
@Injectable({ providedIn: 'root' })
export class SupplyCategoriesDataService {
  private firestoreService = inject(FirestoreService);

  private _resource = resource({
    loader: () => this.firestoreService.getDocuments('cost-types').then(data => data as SupplyCategory[])
  });

  readonly categories = computed(() => this._resource.value() ?? []);
  readonly isLoading = this._resource.isLoading;

  async add(data: SupplyCategory): Promise<string> {
    const ref = await this.firestoreService.addDocument('cost-types', data);
    this._resource.update(list => [...(list ?? []), { ...data, id: ref.id }]);
    return ref.id;
  }

  async update(id: string, data: SupplyCategory): Promise<void> {
