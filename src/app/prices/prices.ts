...
export class Prices implements OnInit {
  ...
  // Costos base por unidad (sin márgenes)
  doughCostPerUnit = computed(() => {
    return this.doughIngredients().reduce((sum, ing) => sum + ing.totalCost, 0);
  });

  recipeCostPerUnit = computed(() => {
    return this.recipeIngredients().reduce((sum, ing) => sum + ing.totalCost, 0);
  });

  deliveryCostPerUnit = computed(() => {
    return this.deliveryItems().reduce((sum, item) => sum + item.totalCost, 0);
  });

  baseCostPerUnit = computed(() => {
    return this.doughCostPerUnit() + this.recipeCostPerUnit() + this.deliveryCostPerUnit();
  });

  // Costos base totales (sin márgenes)
  doughCost = computed(() => {
    return this.doughCostPerUnit() * this.quantity();
  });

  recipeCost = computed(() => {
    return this.recipeCostPerUnit() * this.quantity();
  });

  deliveryCost = computed(() => {
    return this.deliveryCostPerUnit() * this.quantity();
  });

  baseCost = computed(() => {
    return this.baseCostPerUnit() * this.quantity();
  });

  // Costos con márgenes aplicados (por unidad)
  doughCostWithMargin = computed(() => {
    return this.doughIngredients().reduce((sum, ing) => {
      const costWithMargin = ing.totalCost * (1 + ing.margin / 100);
  ...
  deliveryCostWithMargin = computed(() => {
    return this.deliveryItems().reduce((sum, item) => {
      const costWithMargin = item.totalCost * (1 + item.margin / 100);
      return sum + costWithMargin;
    }, 0);
  });
  ...
  // Margen total aplicado (diferencia entre precio final y costo base) - por unidad
  marginAmount = computed(() => {
    return this.totalCostPerUnit() - this.baseCostPerUnit();
  });

  // Calcular el porcentaje de margen total promedio aplicado
  totalMarginPercentage = computed(() => {
    const base = this.baseCostPerUnit();
    if (base === 0) return 0;
